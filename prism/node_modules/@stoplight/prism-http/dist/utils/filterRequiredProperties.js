...
const buildSchemaFilter = (keepPropertyPredicate) => {
    function filterProperties(schema) {
        return (0, function_1.pipe)(O.fromNullable(schema.properties), O.map(properties => (0, function_1.pipe)(Object.keys(properties), A.reduce({}, (filteredProperties, propertyName) => {
            return (0, function_1.pipe)(properties[propertyName], O.fromPredicate(p => {
                if (typeof p === 'boolean') {
    ...
    function filter(inputSchema) {
        return (0, function_1.pipe)(inputSchema, keepPropertyPredicate, O.chain(inputSchema => filterProperties(inputSchema)), O.chain(schema => filterRequired(schema, inputSchema)));
    }
    return filter;
};
