...
const helpers = {
    negotiateByPartialOptionsAndHttpContent({ code, exampleKey, dynamic }, httpContent) {
        ...
        if (exampleKey) {
            return (0, function_1.pipe)((0, InternalHelpers_1.findExampleByKey)(httpContent, exampleKey), E.fromOption(() => types_1.ProblemJsonError.fromTemplate(errors_1.NOT_FOUND, `Response for contentType: ${mediaType} and exampleKey: ${exampleKey} does not exist.`)), E.map(bodyExample => ({ code, mediaType, bodyExample, schema })));
        }
        else if (dynamic) {
    ...
    negotiateDefaultMediaType(partialOptions, response) {
        ...
        return (0, function_1.pipe)(findHttpContent, O.fold(() => E.right({
            ...
            headers,
        }), content => (0, function_1.pipe)(helpers.negotiateByPartialOptionsAndHttpContent({ code, dynamic, exampleKey }, content), E.map(contentNegotiationResult => ({ headers, ...contentNegotiationResult })))));
    },
